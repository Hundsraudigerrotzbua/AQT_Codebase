...
total_time = 1  # 100ms
cw_time_sec = 50e-3  # 50us
cw_time_cycles = int(cw_time_sec * 1e9 // 4)
n_count = 100 #int(total_time / cw_time_sec)
...
while vec_handle.is_processing():
    try:
        ...
        a = np.roll(a, -1)
        a[-1] = new_counts['value'] / 50e-3
        b = np.round(np.mean(a), 3)
        c = np.round(np.std(a), 3)
        counts.append(new_counts['value'] / 50e-3)
...
np.savez('messung.npz', [time, counts])
