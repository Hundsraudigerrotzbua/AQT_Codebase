...
laser_len_cycles = 6000 // 4  # laser duration length (units of clock cycles = 4 ns)
mw_len_cycles = 80 // 4  # MW duration length (units of clock cycles = 4 ns)
meas_len = laser_len_cycles * 4 + 1000  # total meas length (ns)
...
with program() as calib_delays:
    ...
    j = declare(int)  # variable used to save data
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        play('laser_ON', 'AOM', duration=laser_len_cycles)  # Photoluminescence

        #wait(initial_delay_cycles + (laser_len_cycles//2), 'NV')  # delay the microwave pulse

        #wait(initial_delay_cycles - (mw_len_NV // 4), 'NV')  # delay the microwave pulse
        #play('pi', 'NV')#, duration=mw_len_cycles)  # play microwave pulse

        measure('readout', 'APD', None,
                time_tagging.analog(times, meas_len, counts))  # photon count on SNSPD
        wait(wait_between_runs, 'APD')
    ...
    with stream_processing():
        times_st.buffer(100).flatten().histogram([[i, i + 9] for i in range(0, meas_len, 10)]).save(
            'times_hist')  # histogram
        n_st.save('iteration')
...
if simulate:
    ...
    while times_hist_handle.is_processing():
        try:
            ...
            plt.plot(t_vec[::10] + 10 / 2, times_hist / 1000 / (meas_len * 1e-9) / iteration)  # kcps
            plt.xlabel('t [ns]')
    ...
    times_hist = times_hist_handle.fetch_all()
    plt.plot(t_vec[::10] + 10 / 2, times_hist / 1000 / (meas_len * 1e-9) / n_avg)  # kcps vs. meas_len
    plt.xlabel('t [ns]')
    # plt.ylabel(f'counts [kcps / {meas_len / 1000}us]')
    plt.ylabel(f'counts [kcps]')
    plt.title('Delays')
    np.savez('counts_no_mw.npz', times_hist)
